...
Another possibility is to provide your own serializers, see <<_serialization>>.
...
----
<1> Necessary annotation, that this Saga is registered with field for serialization, if the serializer cannot access fields(see <<_serialization>>)
...
NOTE: The dependency quarkus-axon-transaction for transaction management is automatically added. For detailed information see <<Transaction>>.
