...
If you decide to provode your own Aggregate-Configurer, which creates, Axon's `AggregateConfiguration` you can implement the interface `at.meks.quarkiverse.axon.runtime.QuarkusAggregateConfigurer`.
...
==== JDBC Tokenstore
If you would like to use the JDBC Tokenstore you simply have to add the dependency to the extension, and setup a datasource in quarkus.
