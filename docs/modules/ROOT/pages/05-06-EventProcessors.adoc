...
link:05-05-CustomEventSourcedEntityConfigurer.adoc[← Previous: Custom Event Sourced Entity Configurer]
...
=== Default values
For the pooled processor you can set properties for the default pooled processor using the properties of 'quarkus.axon.pooledprocessor' without a processor name. +
Those values are also used as default for the other defined tracking processors.

In the following example a default pooled processor is defined with a batch size of 100 and the initial position tail. +
...
CAUTION: Persistent Streams are currently not supported, because they were not supported in the Axon Framework in version 5.1.0.
...
quarkus.axon.subscribingprocessor.namespaces=GiftCardInMemory
----
