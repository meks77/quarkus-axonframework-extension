...
public class CardReturnSaga {
    ...
    @Inject
    private CardReturnService cardReturnService; // <1>

    @StartSaga
    @SagaEventHandler(associationProperty = "id", keyName = "cardId")
    void handle(Api.CardGotEmptyEvent event) {
        cardReturnService.cardBecameEmpty(); // <2>
    }

    @EndSaga
    @SagaEventHandler(associationProperty = "id", keyName = "cardId")
    void handle(Api.CardReturnedEvent event) {
        cardReturnService.cardWasReturned();// ... <3>
    }
}
----
<1> An application scoped is injected
<2> the application scoped bean is used to do some necessary task
