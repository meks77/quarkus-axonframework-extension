...
public class OpenTelemetryConfigurer implements AxonTracingConfigurer {
    ...
    /**
     * if the configuration of the framework is too fast, it seems that a concurrency problem sometimes leads to the error:
     * GlobalOpenTelemetry.set has already been called. GlobalOpenTelemetry.set must be called only once before any calls to
     * GlobalOpenTelemetry.get. If you are using the OpenTelemetrySdk, use OpenTelemetrySdkBuilder.buildAndRegisterGlobal
     * instead. Previous invocation set to cause of this exception.
     * <p>
     * Currently, the error occurred when execution the tests. But I am not sure if it doesn't occur in production too.
     * <p>
     ...
     * <p>
     * As far as I can see, the error happens in the opentelemetry source in at io.opentelemetry.api.GlobalOpenTelemetry.get
     * at the release 1.44.1 in line 85 when setting Opentelemtry.noop() as instance. But why should it be a noop when it's
     * activated?
     * <p>
     * The source of Opentelemetry seems to be thread-safe. Maybe the Quarkus Extension isn't. Maybe sometimes it's initialized
     * too late.
