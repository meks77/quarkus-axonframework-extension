...
If you want to configure only the serialization of Axon releated data, you can implement `AxonConverterProducer`.
...
In the case snapshotting is activated and the Event Sourced Entities and Entities hide their fields, which is a common pattern, the field visibility ANY must be configured in the object mapper for the serializer(AxonSerializerProducer.createSerializer).
